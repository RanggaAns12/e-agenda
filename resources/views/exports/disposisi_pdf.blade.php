...
<body>
    ...
    <!-- Title -->
    <div class="title">Laporan Data Disposisi Surat</div>
        ...
        <tbody>
            @forelse($disposisi as $index => $disp)
                <tr>
                    ...
                    <td>
                        @if($disp->suratMasuk)
                            No. Agenda: <strong>{{ $disp->suratMasuk->no_agenda }}</strong><br>
                            No. Surat: {{ $disp->suratMasuk->no_surat }}
                        @else
                            <em style="color: #999;">Surat Terhapus</em>
                        @endif
                    </td>
                    <td>{{ $disp->suratMasuk->asal_surat ?? '-' }}</td>
                    ...
                    <td class="text-center"><strong>{{ strtoupper($disp->status) }}</strong></td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center" style="color: #999;">Tidak ada data disposisi surat.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
