<x-app-layout>
    <div x-data="{ showDeleteModal: false, deleteUrl: '' }" class="animate-fade-in-up space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-extrabold text-gray-800 tracking-tight">Data Surat Keluar</h2>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($suratKeluar as $surat)
                            <tr class="hover:bg-green-50/30 transition-colors">
                                <td class="px-6 py-4 font-bold text-gray-800">{{ $surat->no_surat }}</td>
                                <td class="px-6 py-4">
                                    <span class="block text-sm font-semibold text-gray-800">{{ $surat->tujuan_surat }}</span>
                                    <span class="block text-xs text-gray-500 truncate max-w-[200px]">{{ $surat->perihal }}</span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 flex justify-center gap-3">
                                    <a href="{{ route('surat-keluar.show', $surat->id) }}" class="text-blue-500 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 p-2 rounded-lg transition-colors" title="Detail">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                    </a>
                                    <a href="{{ route('surat-keluar.edit', $surat->id) }}" class="text-amber-500 hover:text-amber-700 bg-amber-50 hover:bg-amber-100 p-2 rounded-lg transition-colors" title="Edit">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    </a>
                                    <button @click="showDeleteModal = true; deleteUrl = '{{ route('surat-keluar.destroy', $surat->id) }}'" class="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-2 rounded-lg transition-colors" title="Hapus">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-10 text-center text-gray-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-base font-medium text-gray-500">Belum ada data surat keluar.</p>
                                    </div>
                                </td>
                            </tr>
                        @endempty
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                <h3 class="text-lg font-bold leading-6 text-gray-900" id="modal-title">Hapus Surat Keluar</h3>
                                <div class="mt-2">
        ...
        @if (session('success') || session('error') || $errors->any())
            <div x-data="{ showNotification: true }" x-show="showNotification" style="display: none;" class="fixed inset-0 z-[999] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                <div class="flex min-h-screen items-center justify-center px-4 text-center sm:p-0">
                    
                    <div x-show="showNotification" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="fixed inset-0 bg-gray-900 bg-opacity-60 backdrop-blur-sm transition-opacity" @click="showNotification = false"></div>

                    <div x-show="showNotification" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative inline-block transform overflow-hidden rounded-2xl bg-white text-left align-bottom shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:align-middle">
