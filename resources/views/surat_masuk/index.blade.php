<x-app-layout>
    <div x-data="{ showDeleteModal: false, deleteUrl: '' }" class="space-y-6">
        ...
        <div class="animate-fade-in-up space-y-6">
            ...
            <!-- Filter & Pencarian -->
            <form id="filter-form" method="GET" action="{{ route('surat-masuk.index') }}" class="flex flex-col md:flex-row gap-3 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
                <div class="flex-1 relative">
                    ...
                    <input type="text" id="search-input" name="search" value="{{ request('search') }}" placeholder="Cari nomor agenda, no. surat, asal surat..." class="block w-full pl-10 pr-4 py-2.5 text-sm rounded-xl border border-gray-200 focus:border-[#15803d] focus:ring focus:ring-green-200/50 transition-all placeholder:text-gray-400">
                </div>
                ...
                <div class="flex gap-2">
                    ...
                    <div id="reset-button-container" class="inline-flex">
                        @if(request('search') || request('kode_klasifikasi'))
                            <a id="reset-link" href="{{ route('surat-masuk.index') }}" class="inline-flex items-center justify-center rounded-xl bg-gray-100 hover:bg-gray-200 px-4 py-2.5 text-sm font-bold text-gray-600 transition-all">
                                Reset
                            </a>
                        @endif
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.exportData = function(type) {
            ...
            if (form) {
                ...
                const queryString = params.toString();
                const baseUrl = type === 'excel' ? '{{ route("surat-masuk.export.excel") }}' : '{{ route("surat-masuk.export.pdf") }}';
