<x-app-layout>
    <div class="animate-fade-in-up space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold text-gray-800 tracking-tight">Edit Surat Masuk</h2>
                <p class="text-sm text-gray-500 mt-1">Perbarui informasi untuk data surat masuk yang dipilih.</p>
            </div>
            ...
            <form action="{{ route('surat-masuk.update', $suratMasuk->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="no_agenda" id="no_agenda" value="{{ old('no_agenda', $suratMasuk->no_agenda) }}" required
                    ...
                    <div>
                        ...
                        <select name="kode_klasifikasi" id="kode_klasifikasi" required
                            ...
                            @foreach ($klasifikasi as $klas)
                                <option value="{{ $klas->kode_klasifikasi }}" {{ old('kode_klasifikasi', $suratMasuk->kode_klasifikasi) == $klas->kode_klasifikasi ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="date" name="tgl_surat" id="tgl_surat" value="{{ old('tgl_surat', $suratMasuk->tgl_surat) }}" required
                    ...
                    <div>
                        ...
                        @if ($suratMasuk->file_surat)
                            <div class="mt-2.5 flex items-center gap-2 p-2.5 rounded-xl bg-gray-50 border border-gray-100 text-xs text-gray-600">
                                ...
                                <a href="{{ asset('storage/' . $suratMasuk->file_surat) }}" target="_blank" class="font-bold text-[#15803d] hover:underline">
                ...
                <div>
                    ...
                    <textarea name="isi_ringkas" id="isi_ringkas" rows="4" required
                              ...
                              placeholder="Tuliskan ringkasan isi surat atau perihal surat disini...">{{ old('isi_ringkas', $suratMasuk->isi_ringkas) }}</textarea>
                    @error('isi_ringkas')
                        <p class="text-red-500 text-xs font-semibold mt-1">{{ $message }}</p>
                    @enderror
                </div>
