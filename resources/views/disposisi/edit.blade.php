<x-app-layout>
    <div class="animate-fade-in-up space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold text-gray-800 tracking-tight">Edit Disposisi</h2>
                <p class="text-sm text-gray-500 mt-1">Perbarui instruksi disposisi surat masuk.</p>
            </div>
            ...
            <form action="{{ route('disposisi.update', $disposisi->id) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="surat_masuk_id" id="surat_masuk_id" required
                            ...
                            @foreach ($suratMasuk as $surat)
                                <option value="{{ $surat->id }}" {{ old('surat_masuk_id', $disposisi->surat_masuk_id) == $surat->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="text" name="tujuan_disposisi" id="tujuan_disposisi" value="{{ old('tujuan_disposisi', $disposisi->tujuan_disposisi) }}" required
                    ...
                    <div>
                        ...
                        <input type="date" name="tgl_disposisi" id="tgl_disposisi" value="{{ old('tgl_disposisi', $disposisi->tgl_disposisi) }}" required
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="Diteruskan" {{ old('status', $disposisi->status) == 'Diteruskan' ? 'selected' : '' }}>Diteruskan</option>
                            <option value="Proses" {{ old('status', $disposisi->status) == 'Proses' ? 'selected' : '' }}>Proses</option>
                            <option value="Selesai" {{ old('status', $disposisi->status) == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                            <option value="Ditolak" {{ old('status', $disposisi->status) == 'Ditolak' ? 'selected' : '' }}>Ditolak</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-xs font-semibold mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea name="isi_disposisi" id="isi_disposisi" rows="4" required
                              ...
                              placeholder="Tuliskan instruksi atau disposisi detail dari pimpinan...">{{ old('isi_disposisi', $disposisi->isi_disposisi) }}</textarea>
                    @error('isi_disposisi')
                        <p class="text-red-500 text-xs font-semibold mt-1">{{ $message }}</p>
                    @enderror
                </div>
